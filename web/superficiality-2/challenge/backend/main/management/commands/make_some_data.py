...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for _ in range(NO_OF_USERS // 2):
            ...
            jeff_friends.append(new_friend)
        ...
        # make some private
        logger.info("Turning some users as private ...")
        for _ in range(len(jeff_friends) // 3):
            some_friend = random.sample(jeff_friends, 1)[0]
            some_friend.is_private = True
            some_friend.save()
            logger.info(f"Made {some_friend.full_name} private")
        ...
        for _ in range(3):
            post = Post.objects.create(
                user=jeff,
                message=faker.paragraph(nb_sentences=random.randint(1, 5)),
                is_published=True,
            )
