    ...
    addNewPost(message) {
      ...
      makeNewPost(token, this.activeUserId, message)
        .then((data) => {
          ...
          newPostData.isPublished = data.ispublished;
          newPostData.uuid = data.uuid;

          // redirect to publishing
          this.$router.push({name: 'publish-post', params: {'postId': data.postid}})
        })
        .catch((error) => {
...
<template>
  <div class="my-5">
    ...
    <div class="border rounded-2 bg-dark p-5 fs-5 text-center" v-if="activeUser !== null && activeUser.isPrivate">
      <p class="text-danger m-0">This user has chosen to have a private profile</p>
    </div>
