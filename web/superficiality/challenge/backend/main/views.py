...
def check_jwt_authentication(view):
    """
    Only checks if there is a user with the ID present in the JWT payload.
    Part one of the authorization vulnerability,
    since the function does not check if the JWT actually
    belongs to the user
    """
            ...
            if 'Bearer' in AUTHORIZATION_VALUE:
                ...
                if len(parts) == 2:
                    ...
                    if len(token_parts) == 3:
                        ...
                        # by not making sure to which user this token belongs

                        if User.objects.filter(current_jwt_signature=signature).exists():
                            ...
                            try:
                                ...
                                # this is the first part of the flaw
                                # because this checker does not
                                # look if the jwt can be decoded successfully
                                # using the auth_key set on the user
...
def feed(request, userid):
    ...
    feed_posts = Post.objects.filter(user__in=feed_users).order_by('created_at')
...
def handle_new_post_for_challenge(new_post, result):
    ...
    if new_post.message != "I_HAVE_PWNED_THIS_PLATFORM":
        return
...
def posts(request, userid):
    ...
    if request.method == 'POST':
        ...
        result = serialize_post(new_post)

        handle_new_post_for_challenge(new_post, result)
...
def post(request, userid, postid):
    ...
    return JsonResponse({'postid': post.id, 'message': post.message})
